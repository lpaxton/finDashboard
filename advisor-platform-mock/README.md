...
A dependency-free mock of the draft Advisor Platform API for the firm, advisor and client-portal views. It implements all 53 operations in `openapi.yaml` on one consistent dataset, enforces roles, and serves the dashboard.
...
runs 69 tests, including one that reads `openapi.yaml` and checks that every listed operation is served, and one that checks the dashboard's embedded mock has not drifted from `src/mock-core.js`.
